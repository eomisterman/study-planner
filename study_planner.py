...
def format_duration_for_display(minutes):
    """Format duration in minutes for display (MM:SS format)."""
    mins = int(minutes)
    secs = int((minutes - mins) * 60)  # Handle any fractional minutes
    return f"{mins}:{secs:02d}"


def generate_markdown(scheduled_days, course_data, daily_limit, start_date):
    ...
    try:
        ...
        # Course title header
        lines.append(f"# {course_data['course_title']} - Study Schedule")
        lines.append("")
        
        # Schedule metadata
        lines.append("## Schedule Information")
        lines.append(f"- **Start Date:** {start_date.strftime('%A, %B %d, %Y')}")
        lines.append(f"- **Daily Study Limit:** {daily_limit} minutes ({daily_limit/60:.1f} hours)")
        lines.append(f"- **Total Course Duration:** {course_data['total_duration_minutes']} minutes ({course_data['total_duration_minutes']/60:.1f} hours)")
        lines.append(f"- **Scheduled Days:** {len(scheduled_days)} weekdays")
        lines.append("")
        
        # Progress tracking section
        lines.append("## Progress Tracking")
        lines.append("Check off items as you complete them. Each day's progress is tracked separately.")
        lines.append("")
        ...
        for day in scheduled_days:
            ...
            day_header = f"### Day {day['day_number']} - {day['date_string']} ({day['total_minutes']} minutes)"
            lines.append(day_header)
            lines.append("")
            
            # Items for this day
            for item in day['items']:
                # Format item with checkbox
                duration_display = format_duration_for_display(item['duration_minutes'])
                item_line = f"- [ ] {item['title']} ({duration_display})"
                lines.append(item_line)
                
                # Add section context for non-split items (to help with navigation)
                if not item.get('is_split', False):
                    lines.append(f"      *Section: {item['section_title']}*")
            
            lines.append("")  # Empty line after each day
        ...
        total_scheduled_minutes = sum(day['total_minutes'] for day in scheduled_days)
        total_items = sum(len(day['items']) for day in scheduled_days)
        split_items_count = sum(1 for day in scheduled_days for item in day['items'] if item.get('is_split', False))
        avg_minutes_per_day = total_scheduled_minutes / len(scheduled_days)
        
        lines.append(f"- **Total Study Days:** {len(scheduled_days)}")
        lines.append(f"- **Total Study Time:** {total_scheduled_minutes} minutes ({total_scheduled_minutes/60:.1f} hours)")
        lines.append(f"- **Total Items:** {total_items}")
        lines.append(f"- **Average Time per Day:** {avg_minutes_per_day:.1f} minutes")
        if split_items_count > 0:
            lines.append(f"- **Split Items:** {split_items_count} (due to oversized content)")
        
        # Add completion dates
        start_date_obj = scheduled_days[0]['date']
        end_date_obj = scheduled_days[-1]['date']
        lines.append(f"- **Completion Date:** {end_date_obj.strftime('%A, %B %d, %Y')}")
        
        # Calculate weekdays between start and end
        total_calendar_days = (end_date_obj - start_date_obj).days + 1
        lines.append(f"- **Calendar Duration:** {total_calendar_days} days ({total_calendar_days/7:.1f} weeks)")
        lines.append("")
        
        # Tips section
        lines.append("## Study Tips")
        lines.append("- [ ] Set up a consistent study environment")
        lines.append("- [ ] Take notes during video content")
        lines.append("- [ ] Review previous day's material before starting new content")
        lines.append("- [ ] Take breaks every 25-30 minutes (Pomodoro technique)")
        lines.append("- [ ] Track your progress by checking off completed items")
        lines.append("")
        
        # Footer
        lines.append("---")
        lines.append("*Generated by Study Planner*")
        
        markdown_content = "\n".join(lines)
        
        print(f"   ✅ Markdown generated: {len(lines)} lines")
        print(f"   📊 Content: {len(scheduled_days)} days, {total_items} items")
...
Output:
  ...
  Content is balanced across days respecting your time limits
    ...
    try:
        ...
        markdown = generate_markdown(scheduled_days, course_data, validated_daily_limit, validated_start_date)
